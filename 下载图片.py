import os
import requests
from utils.index import get_number_first
import base64
...
def parsing_picture(url,address):
    create_folder(address)
    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/110.0.0.0 Safari/537.36'
    }
    picture_down = requests.get(url=url,headers=headers)
    with open(address + get_number_first(url) + '.jpg', mode='wb') as f:
        f.write(picture_down.content)
    with open(address + get_number_first(url) + '.jpg', mode='rb') as f:
